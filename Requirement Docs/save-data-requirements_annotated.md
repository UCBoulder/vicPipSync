...
Requirements written by Claude 3.7 Sonnet Concise in response to the following prompt:

    I'm writing a function in MATLAB, and need a robust requirements document. Please look at the function stub (included below) and write the requirements document in .md format. Please prefer the use of native functions over writing new ones, and focus on best practices and human readability of the code. The function should fail gracefully, and work with any OS's native file saving/directory exploring functionality. Here is the function stub:
